# Test Plan: POST /api/tags & DELETE /api/tags/{tagId}

This document outlines the test strategy and test cases to cover the `/api/tags` endpoints based on the AAA (Arrange, Act, Assert) principle.
...
*   **Test**: `should successfully create a new tag`
    ...
    *   **Assert**: Status `201 Created`, validate schema (`TagSchema`), verify slug generation.

*   **Test**: `should return 400 Bad Request if the name is missing or not a string`
    *   **Arrange**: ADMIN token, payload `{}` or `{ name: 123 }`.
    *   **Act**: Send `POST` request.
    *   **Assert**: Status `400 Bad Request`, validate headers.
...
*   **Test**: `should return 409 Conflict if a tag with the same slug already exists`
    ...
    *   **Assert**: Status `409 Conflict`, validate headers.

*   **Test**: `should return 401 Unauthorized if no token is provided` (Status 401).
*   **Test**: `should return 403 Forbidden if user role is not ADMIN` (Status 403).
...
*   **Test**: `should successfully delete an existing tag`
    ...
    *   **Assert**: Status `200 OK`, validate response `{ success: true }`.
